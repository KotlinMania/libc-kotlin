...
// stdio.h (FILE* params — FILE is project class, no cinterop conflict)
...
// stdlib.h — string-returning functions (no COpaquePointer params)
public actual fun system(s: String?): CInt = system(s)
public actual fun getenv(s: String?): String? = getenv(s)

// string.h — string-only functions (no COpaquePointer params)
...
// COpaquePointer functions (calloc, malloc, realloc, free, memchr, memcmp,
// memcpy, memmove, memset) — blocked on type bridge between project's
// COpaquePointer class and kotlinx.cinterop.COpaquePointer. See session notes.