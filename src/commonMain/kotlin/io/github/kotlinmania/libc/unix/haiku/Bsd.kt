...
/**
 * BSD APIs available in Haiku (`headers/compatibility/bsd`).
 */

public typealias StringList = Stringlist

public data class Stringlist(
    val slStr: List<String>?,
    val slMax: ULong,
    val slCur: ULong,
)
...
public data class Kevent(
    ...
    init {
        require(ext.size == 4) { "ext must be 4 entries" }
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        other as Kevent
        return ident == other.ident && filter == other.filter && flags == other.flags &&
            fflags == other.fflags && data == other.data && udata == other.udata &&
            ext.contentEquals(other.ext)
    }

    override fun hashCode(): Int = 31 * ident.hashCode() + ext.contentHashCode()
}
...
public const val EV_ONESHOT: UShort = 0x0010u
...
public expect fun strsep(string: String?, delimiters: String?): String?
...
public expect fun dlIteratePhdr(
    callback: ((info: DlPhdrInfo?, size: ULong, data: COpaquePointer?) -> CInt)?,
    data: COpaquePointer?,
): CInt
