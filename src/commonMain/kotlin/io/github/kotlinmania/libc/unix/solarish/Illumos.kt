...
public data class Aiocb(
    ...
    init {
        require(aioPad.size == 1) { "aio__pad must be 1 entry" }
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return aioFildes == other.aioFildes && aioBuf == other.aioBuf && aioNbytes == other.aioNbytes &&
            ...
            aioPad.contentEquals(other.aioPad)
    }

    override fun hashCode(): Int = 31 * aioFildes + aioPad.contentHashCode()
}
...
public data class ShmidDs(
    ...
    init {
        require(shmPad4.size == 4) { "shm_pad4 must be 4 entries" }
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other == null || this::class != other::class) return false
        other as ShmidDs
        return shmPerm == other.shmPerm && shmSegsz == other.shmSegsz && shmAmp == other.shmAmp &&
            ...
            shmDtime == other.shmDtime && shmCtime == other.shmCtime && shmPad4.contentEquals(other.shmPad4)
    }
...
public data class FilInfo(
    ...
    init {
        require(fiName.size == FILNAME_MAX) { "fi_name must be $FILNAME_MAX bytes" }
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other == null || this::class != other::class) return false
        other as FilInfo
        if (fiFlags != other.fiFlags) return false
        if (fiPos != other.fiPos) return false
        if (!fiName.contentEquals(other.fiName)) return false
        return true
    }

    override fun hashCode(): Int {
        var result = fiFlags
        result = 31 * result + fiPos
        result = 31 * result + fiName.contentHashCode()
        return result
    }
}

// repr(packed(4)) on x86/x86_64.
...
public data class Utmpx(
    ...
    init {
        require(utUser.size == _UTX_USERSIZE)
        require(utId.size == _UTX_IDSIZE)
        require(utLine.size == _UTX_LINESIZE)
        require(utPad.size == _UTX_PADSIZE)
        require(utHost.size == _UTX_HOSTSIZE)
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return utUser.contentEquals(other.utUser) && utId.contentEquals(other.utId) &&
            ...
            utPad.contentEquals(other.utPad) && utSyslen == other.utSyslen && utHost.contentEquals(other.utHost)
    }
...
public const val _UTX_PADSIZE: Int = 5
public const val _UTX_IDSIZE: Int = 4
public const val _UTX_HOSTSIZE: Int = 257
...
public const val FILNAME_MAX: Int = 32
...
public const val VSTATUS: Int = 16
public const val VERASE2: Int = 17
...
// sys/timerfd.h — octal in upstream, written here in hex.
public const val TFD_CLOEXEC: Int = 0x80000 // 0o2000000
public const val TFD_NONBLOCK: Int = 0x800 // 0o4000
