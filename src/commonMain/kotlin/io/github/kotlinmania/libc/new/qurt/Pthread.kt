...
import io.github.kotlinmania.libc.new.qurt.sys.CpuSetT
import kotlinx.cinterop.COpaquePointer

/**
 * Header: `pthread.h`.
 *
 * QuRT provides a subset of POSIX pthread functionality for real-time systems.
 */
...
public const val PTHREAD_MUTEX_INITIALIZER: PthreadMutexT = 0xFFFFFFFFu
public const val PTHREAD_COND_INITIALIZER: PthreadCondT = 0xFFFFFFFFu
public const val PTHREAD_ONCE_INIT: PthreadOnceT = 0u
...
public expect fun pthreadCreate(
    thread: PthreadT?,
    attr: PthreadAttrT?,
    startRoutine: (COpaquePointer?) -> COpaquePointer?,
    arg: COpaquePointer?,
): CInt
...
public expect fun pthreadCondTimedwait(
    cond: PthreadCondT?,
    mutex: PthreadMutexT?,
    abstime: Timespec?,
): CInt
...
public expect fun pthreadKeyCreate(
    key: PthreadKeyT?,
    destructor: ((COpaquePointer?) -> Unit)?,
): CInt
...
public expect fun pthreadAttrSetaffinityNp(
    attr: PthreadAttrT?,
    cpusetsize: ULong,
    cpuset: CpuSetT?,
): CInt

public expect fun pthreadAttrGetaffinityNp(
    attr: PthreadAttrT?,
    cpusetsize: ULong,
    cpuset: CpuSetT?,
): CInt
