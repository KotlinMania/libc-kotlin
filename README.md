...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`libc`](https://crates.io/crates/libc).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `libc` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Raw FFI bindings to platform libc
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:libc-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `libc` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
