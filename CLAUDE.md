# Claude Code Project Instructions — libc-kotlin

## Project Overview

This is **libc-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`libc`](https://crates.io/crates/libc).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/libc/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/libc>
